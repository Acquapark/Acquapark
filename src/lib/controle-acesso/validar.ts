...
async function validarIngresso(supabase: SupabaseClient, ingresso: Record<string, unknown>): Promise<ValidacaoResultado> {
...
async function validarCredencial(supabase: SupabaseClient, credencial: Record<string, unknown>): Promise<ValidacaoResultado> {
...
/**
 * Núcleo da validação de um código de QR (ingresso ou credencial) — usado
 * tanto pela tela Controle de Acesso (`validarCodigo`, sessão de
 * funcionário) quanto pela rota `/api/catraca/validar` (chave de API, sem
 * sessão): mesma regra de negócio nos dois lugares, só muda quem autentica
 * a chamada e qual cliente Supabase é usado.
 */
export async function validarPorCodigo(supabase: SupabaseClient, codigoBruto: string): Promise<ValidacaoResultado> {
  ...
  if (ingresso) {
    return validarIngresso(supabase, ingresso as unknown as Record<string, unknown>);
  }
